...
void HandleFlags(ProcessorState *emulator, uint16_t result, bool carry, bool aux){
...
void ProcessOpcode(unsigned char *memory, int pc, ProcessorState *emulator){
    ...
    if (*instruction == 0xd3) { // OUT
        OutputToPort(instruction[1], emulator->a);
        emulator->pc += 1;
    }
    ...
    switch (*instruction){
        ...
        case 0x04: // INR B
        {
            ...
            HandleFlags(emulator, result, false, true);
        ...
        case 0x07: // RLC
        {
            uint8_t result = emulator->a;
            emulator->a = ((result & 0x80) >> 7) | (result << 1);
            emulator->flags.cy = (0x80 == (result&0x80));
            break;
        }
        case 0x09: // DAD B
        {
            uint32_t hl = (emulator->hl[1] << 8) | emulator->hl[0];
			uint32_t bc = (emulator->bc[1] << 8) | emulator->bc[0];
			uint32_t res = hl + bc;
			emulator->hl[1] = (res & 0xff00) >> 8;
			emulator->hl[0] = res & 0xff;
			emulator->flags.cy = ((res & 0xffff0000) != 0);
            break;
        }
        case 0x0a: // LDAX B
        {
            emulator->a = emulator->memory[emulator->bc_pair];
            break;
        }
        case 0x0b: // DCX B 
        {
            emulator->bc_pair = emulator->bc_pair-1;
            break;
        }
        ...
        {
            uint8_t result = emulator->a;
            emulator->a = ((result & 1) << 7) | (result >> 1);
            emulator->flags.cy = (1 == (result&1));
            break;
        }
        case 0x11: // LXI D
        {
        ...
        case 0x14: 
        {
            uint16_t result = emulator->de[1] +1;
        ...
        {
			uint32_t hl = (emulator->hl[1] << 8) | emulator->hl[0];
			uint32_t de = (emulator->de[1] << 8) | emulator->de[0];
			uint32_t res = hl + de;
			emulator->hl[1] = (res & 0xff00) >> 8;
			emulator->hl[0] = res & 0xff;
			emulator->flags.cy = ((res & 0xffff0000) != 0);
        ...
        {
            uint8_t result = emulator->a;
            emulator->a = ((emulator->flags.cy) << 7) | (result >> 1);
            emulator->flags.cy = (1 == (result&1));
            break;
        }
        ...
        case 0x22: // SHLD
        {
            ...
            emulator->memory[emulator->combined] = emulator->hl[0];
            emulator->memory[emulator->combined+1] = emulator->hl[1];
            emulator->pc += 2;
        }
        case 0x23: // INX H
        {
        ...
        case 0x24: // INR H
        {
            uint8_t result = emulator->hl[1] + 1;
            emulator->hl[1] = result;
        ...
        case 0x27: // DAA
        {
            if ((emulator->a &0xf) > 9)
                emulator->a += 6;
            if ((emulator->a&0xf0) > 0x90)
            {
                uint16_t res = (uint16_t) emulator->a + 0x60;
                emulator->a = res & 0xff;
                HandleFlags(emulator,res,res>0xff,true);
            }
            break;
        }
        case 0x29: // DAD H
        ...
        case 0x2a: // LHLD
        {
            emulator->seperate[0] = instruction[1];
            emulator->seperate[1] = instruction[2];
            emulator->hl[0] = emulator->memory[emulator->combined];
            emulator->hl[1] = emulator->memory[emulator->combined+1];
            emulator->pc += 2;
            break;
        }
        case 0x2b: // DCX H
        {
            emulator->hl_pair = emulator->hl_pair -1;
            break;
        }
        ...
        case 0x32: // STA
        {
            emulator->seperate[0] = instruction[1];
            emulator->seperate[1] = instruction[2];
            emulator->memory[emulator->combined] = emulator->a;
            emulator->pc += 2;
            break;
        }
        ...
        case 0x35: // DCR M
        {
            uint16_t result = emulator->memory[emulator->hl_pair] - 1;
            emulator->memory[emulator->hl_pair] = result;
            HandleFlags(emulator,result,false, true);
            break;
        }
        case 0x36: // MVI M
        ...
        case 0x37: // SCF
        {
            emulator->flags.cy = true;
            break;
        }
        case 0x39: // DAD SP
        {
            uint32_t hl = (emulator->hl[1] << 8) | emulator->hl[0];
			uint32_t res = hl + emulator->sp;
			emulator->hl[1] = (res & 0xff00) >> 8;
			emulator->hl[0] = res & 0xff;
			emulator->flags.cy = ((res & 0xffff0000) != 0);
            break;
        ...
        case 0x3a: // LDA
        {
            emulator->seperate[0] = instruction[1];
            emulator->seperate[1] = instruction[2];
            emulator->a = emulator->memory[emulator->combined];
            emulator->pc += 2;
            break;
        }
        case 0x3b: // DCX SP
        ...
        case 0x45: // MOV B,L
        {
            emulator->bc[1] = emulator->hl[0];
            break;
        }
        case 0x46: // MOV B,M
        {
            emulator->bc[1] = emulator->memory[emulator->hl_pair];
            break;
        }
        case 0x47: OpcodeNotWritten(&memory[pc]); break;
        case 0x48: // MOV C,B
        {
            emulator->bc[0] = emulator->bc[1];
            break;
        }
        case 0x49: // MOV C,C
        {
            emulator->bc[0] = emulator->bc[0];
            break;
        }
        case 0x4a: // MOV C,D
        {
            emulator->bc[0] = emulator->de[1];
            break;
        }
        case 0x4b: OpcodeNotWritten(&memory[pc]); break;
        case 0x4c: // MOV C,H
        {
            emulator->bc[0] = emulator->hl[1];
            break;
        }
        ...
        case 0x4f: // MOV C,A
        {
            emulator->bc[0] = emulator->a;
            break;
        }
        case 0x50: OpcodeNotWritten(&memory[pc]); break;
        case 0x51: // MOV D,C
        {
            emulator->de[1] = emulator->bc[0];
            break;
        }
        ...
        case 0x5f: // MOV E,A
        {
            emulator->de[0] = emulator->a;
            break;
        }
        case 0x60: // MOV H,B
        {
            emulator->hl[1] = emulator->bc[1];
            break;
        }
        ...
        case 0x6e: // MOV L,M
        {
            emulator->hl[0] = emulator->memory[emulator->hl_pair];
            break;
        }
        case 0x6f: // MOV L,A
        {
        ...
        case 0x70: // MOV M,B
        {
            emulator->memory[emulator->hl_pair] = emulator->bc[1];
            break;
        }
        ...
        case 0x78: // MOV A,B
        {
            emulator->a = emulator->bc[1];
            break;
        }
        case 0x79: // MOV A,C
        {
            emulator->a = emulator->bc[0];
            break;
        }
        case 0x7a: // MOV A,D
        {
        ...
        case 0x80: // ADD B
        {
            uint16_t result = emulator->a + emulator->bc[1];

            emulator->a = result & 0xff;
            
            HandleFlags(emulator,result,result>0xff, true);
            break;
        }
        ...
        case 0xb6: // ORA M
        {
            uint16_t result = emulator->a | emulator->memory[emulator->hl_pair];
            HandleFlags(emulator,result, result>0xff, true);
            emulator->a = result;
            break;
        }
        ...
        case 0xc6: // ADI D8
        {
            uint16_t result = (uint16_t)emulator->a + (uint16_t)instruction[1];
            HandleFlags(emulator,result, result>0xff, true);
            emulator->a = result & 0xff;
            emulator->pc += 1;
            break;
        ...
        case 0xd2: // JNC adr
        {
            emulator->seperate[0] = instruction[1];
            emulator->seperate[1] = instruction[2];
            emulator->pc = emulator->flags.cy ? emulator->pc+2 : emulator->combined-1;
            break;
        }
        case 0xd3: // OUT
        {
        ...
        {
            // NOT SURE.
            uint8_t h = emulator->hl[1];
            uint8_t l = emulator->hl[0];
            
            emulator->hl[0] = emulator->memory[emulator->sp];
            emulator->hl[1] = emulator->memory[emulator->sp+1];
            
            emulator->memory[emulator->sp] = l;
            emulator->memory[emulator->sp+1] = h;
            break;
        }
    ...
    if(emulator->pc == 0x204b){
        int stop = 1;
    }
    
    if(emulator->sp == 0xfffe){
        int stop = 1;
    }
    
    if(emulator->pc == 0x1439) {
        int stop = 1;
    }
    
    if(emulator->hl_pair == 0xff9d){
        int stop = 1;
    }
    
    if(instruction[0] == 0x9e) {
        int stop = 1;
    }
    
    if(emulator->de_pair == 0x9c20) {
        int stop = 1;
    }
...
void GenerateInterrupt(ProcessorState* emulator, int interrupt_num)    
   {
    ...
    emulator->memory[emulator->sp-2] = emulator->pc_hl[0]-1;
    emulator->memory[emulator->sp-1] = emulator->pc_hl[1];
    emulator->sp -=2;
...
int main (int argc, char** argv) {
    ...
    while(true){
        ...
        if(count >= 5000000 && count % 1000000 == 0) {
            if(interrupt == 1) {
